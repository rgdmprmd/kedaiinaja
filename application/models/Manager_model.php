...
class Manager_model extends CI_Model
{
    ...
    {
        $sql = "SELECT * FROM menu_jenis mj LEFT JOIN category_icon ci ON mj.makananjenis_icon = ci.icon_url WHERE mj.makananjenis_id = {$id}";
    ...
    public function getCategoryIcon($offset = 0, $limit = 0, $search = null)
    {
        $where_search = "";
        if ($search != null) {
            $where_search = " AND icon_name LIKE '%{$search}%'";
        }

        $sql = "SELECT * FROM category_icon WHERE 1 = 1 {$where_search}";

        $total = $this->db->query($sql)->num_rows();

        if ($limit > 0) {
            $sql .= " ORDER BY icon_name ASC LIMIT {$offset}, {$limit} ";
        }
        ...
        if ($data) {
            foreach ($data as $d) {
                $result['id']       = $d['icon_url'];
                $result['text']     = $d['icon_name'];
                $result['status']   = true;
            ...
            $result['id'] = null;
            $result['text'] = 'Oops, icon tidak ditemukan.';
            $result['status'] = '404';
